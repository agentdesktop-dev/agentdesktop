...
impl ScanContext {
    ...
    pub(crate) fn homes(&self) -> &[PathBuf] {
        &self.homes
    }

    /// Empty values are treated as unset, matching how the harnesses read them.
    pub(crate) fn env_path(&self, name: &str) -> Option<PathBuf> {
        self.environment
            .get(name)
    ...
    pub(crate) fn system_path(&self, path: &str) -> Option<PathBuf> {
        self.include_system_paths.then(|| PathBuf::from(path))
    }

    pub(crate) fn current_dir_ancestors(&self, relative: &Path) -> Vec<PathBuf> {
        self.cwd
            .as_ref()
            .into_iter()
    ...
    pub(crate) fn executable_candidates(&self, name: &str) -> Vec<PathBuf> {
    ...
    #[cfg(test)]
    pub(crate) fn isolated(home: PathBuf, cwd: PathBuf) -> Self {
        Self {
    ...
    #[cfg(test)]
    pub(crate) fn with_override(mut self, name: &'static str, value: PathBuf) -> Self {
