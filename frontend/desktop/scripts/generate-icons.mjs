...
function colorAt(pointX, pointY, state) {
  if (state !== "ready") {
    ...
    if (distance <= 5.5) {
      const cutout =
        state === "offline"
          ? Math.abs(pointX - pointY) <= 0.8 ||
            Math.abs(pointX + pointY - 51) <= 0.8
          : (Math.abs(pointX - 25.5) <= 0.75 &&
              pointY >= 21.7 &&
              pointY <= 25.8) ||
...
function createIcon(size, state = "ready") {
  ...
  for (let pixelY = 0; pixelY < size; pixelY += 1) {
    ...
    for (let pixelX = 0; pixelX < size; pixelX += 1) {
      ...
      pixels[pixelOffset] = coveredSamples
        ? Math.round(accumulated[0] / coveredSamples)
        : 0;
      pixels[pixelOffset + 1] = coveredSamples
        ? Math.round(accumulated[1] / coveredSamples)
        : 0;
      pixels[pixelOffset + 2] = coveredSamples
        ? Math.round(accumulated[2] / coveredSamples)
        : 0;
