    ...
    return (
      <div className="page-stack usage-page">
        <section className="card usage-empty">
          ...
          <div>
            <h2>No usage data</h2>
            <p>Gateway usage is not configured or is currently unavailable.</p>
          </div>
        </section>
      </div>
    );
    ...
    <div className="page-stack usage-page">
      ...
      >
        <div className="usage-summary-heading">
          ...
          <div>
            <h2 id="usage-title">Gateway usage</h2>
            <span
        ...
        <dl className="usage-summary-metrics">
          <div className="usage-summary-primary">
            ...
            <dd>{formatEstimatedCost(llmUsage.estimatedCostUsd)}</dd>
          </div>
          <div>
            <dt>Tokens</dt>
        ...
        <fieldset className="usage-controls">
          ...
          <button
            className="button button-secondary usage-clear-filters"
            type="button"
            disabled={!hasFilters}
                      ...
                      <Fragment key={key}>
                        <tr className="usage-summary-row">
                          ...
                          <td data-label="Estimated cost">
                            {formatEstimatedCost(item.estimatedCostUsd)}
                          </td>
                          <td
                            data-label="Tokens"
...
function InteractionDetails({
  ...
  return (
    ...
    >
            ...
            <tbody>
              {interactions.map((interaction) => (
                <tr key={interaction.id}>
                  ...
                  <td>{formatOptionalCost(interaction.estimatedCostUsd)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {nextCursor ? (
        <button
          className="button button-secondary usage-interactions-more"
          type="button"
          disabled={loading}
          onClick={onLoadMore}
        >
...
function formatOptionalCost(value: number | null): string {
  return value === null ? "—" : formatEstimatedCost(value);
}
  ...
  return [...rows].sort((left, right) => {
    ...
    switch (sort.column) {
      ...
      case "estimatedCostUsd":
        comparison = left.estimatedCostUsd - right.estimatedCostUsd;
        break;
      case "totalTokens":
...
function usageNumberTitle(value: number): string | undefined {
