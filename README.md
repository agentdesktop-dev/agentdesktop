...
| --- | --- | --- |
| Discovery | Inventory agents, models, providers, MCP servers, tools, and relevant configuration on every endpoint. | Remote managed macOS reports Claude Code, Claude Desktop, Codex CLI, OpenClaw, VS Code Copilot, configured MCP names/transports, skills, and plugins from fixed user-level roots. Claude Desktop MCP Extensions are included. Windows reports a bounded Claude Code-only inventory; Linux, project, and model/provider discovery are not implemented. |
