...
| Transparent process capture | Supported for standalone and experimental managed `claude` profiles | Unavailable | Unavailable |
...
- Linux capture: private-container coverage validates cgroup v2/nftables TCP/443 redirection, original-destination recovery, bidirectional HBONE forwarding, and UDP/443 denial. Real Gateway smoke paths validate local token rejection/acceptance, HTTP/2 CONNECT interoperability, and managed certificate-derived identity through dynamic TLS interception. A Fedora VM validates installation, trust, process-tree capture, spoofed-header resistance, fail-closed Gateway loss, and cleanup. Production validation still requires existing-firewall compatibility, restart behavior, packaging signatures, and an explicit anti-bypass position.
