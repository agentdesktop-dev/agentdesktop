...
- Phase 5: in progress. Linux native forwarding and standalone transparent capture are validated in a disposable Fedora Workstation VM. All Rust libraries, binaries, and test targets compile warning-free for `x86_64-pc-windows-msvc` from Linux through a pinned `cargo-xwin` workflow. Standalone native forwarding is also runtime-validated in Windows 11 with real MSVC Agent Desktop and Agent Gateway binaries: the connector supervises Gateway, forwards a request over HBONE to a deterministic direct response, reports healthy status, and closes all listeners when Gateway exits. The Windows QEMU harness has a validated Packer definition, unattended Windows 11 Enterprise installation, Microsoft Visual C++ runtime, UEFI state, OpenSSH control, test-signing, host service forwarding, disposable overlays, and a repeatable native smoke script. This development VM intentionally bypasses the Windows TPM and Secure Boot requirements because neither is part of the connector or WFP test boundary. The next slice implements Windows credentials, service lifecycle, trust, and installation before platform capture. macOS development requires Apple hardware: disposable Virtualization.framework VMs for ordinary journeys and a physical Apple Silicon machine for System Extension approval and Network Extension lifecycle validation. CI automation is deferred while these developer-operated environments are established.
...
Implement Windows credential storage, service startup, trust, and packaging. In parallel, prepare the Apple-hardware development environment for native macOS validation.