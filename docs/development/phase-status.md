...
- Phase 2: partial. The connector retains experimental DPoP forwarding while the production enrollment path uses ordinary OAuth for user identity and short-lived mTLS certificates for device identity. Agent Desktop reads an explicit enrollment-service URL from organization bootstrap and managed runtime configuration, generates and protects P-256 enrollment and retry-stable renewal keys, verifies authority fingerprints and leaf-key binding, persists approved credentials, and fails managed startup closed unless reqwest/rustls can present the identity. It checks leaf validity, renews within six hours of expiry, persists and reloads the validated replacement before publishing a new device generation, and independently rotates the upstream pool for OAuth or device credential changes. The Go service terminates TLS 1.3 directly, permits certificate-free initial enrollment, verifies every presented client certificate against the enrollment CA, validates issuer-pinned bearer tokens and P-256 CSRs, persists enrollment and renewal state in PostgreSQL, requires separately scoped administrator authorization for lifecycle actions, reconciles interrupted issuance with stable request identities, and atomically revokes devices and certificates. Rust tests exercise CSR and fingerprint binding, protected draft reuse, renewal timing, certificate-key matching, config validation, and both pool generations. Go tests cover the TLS handshake matrix, request and renewal idempotency, audit, owner isolation, administrator lifecycle actions, reconciliation, transactional completion, provisional-state isolation, and revocation organization isolation, including disposable PostgreSQL 17 integration. Production CA integration, expired-certificate recovery, fail-closed revocation consumption, and Agent Gateway mTLS identity remain incomplete.
...
- Production enrollment still requires protected-key CA integration, expired-certificate recovery, and fail-closed Agent Gateway consumption of revocation state.
