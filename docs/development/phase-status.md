...
- Production enrollment still requires protected-key CA integration, interrupted-issuance reconciliation, Agent Desktop certificate installation, renewal, expired-certificate recovery, administrator rejection/listing, and revocation enforcement.
...
Implement interrupted-issuance reconciliation, then replace the local CA key with a production protected-key adapter. After that, add Agent Desktop key/certificate lifecycle and Agent Gateway mTLS validation before re-running the managed Fedora journey.