...
- Phase 2: partial. Browser Authorization Code with PKCE, DPoP-bound access tokens, refresh rotation, protected credential storage, restart restoration, enrollment fixtures, revocation state, and per-request managed DPoP headers are implemented. Production Agent Gateway identity enforcement is not implemented.
...
- Managed Phase 3 depends on Phase 2. Remote HBONE must use TLS plus per-CONNECT DPoP validation, replay protection, `cnf.jkt` binding, verified user/device context, and connector-credential stripping in Agent Gateway. The standalone local capability must never be used for managed transport.
- Production enrollment requires an authenticated authority, administrator approval and revocation, and an agreed Agent Gateway device-identity contract.
...
Complete Phase 2 in Agent Gateway, then replace managed capture's local capability with authenticated TLS and per-CONNECT DPoP identity. Re-run the managed Fedora journey with attempts to override verified tunnel identity from inner application headers.