...
- Phase 4, partial: a single 36 MB self-contained development Linux installer embeds a tested connector/Agent Gateway set and starter configuration, verifies independently compressed payloads, presents a guided per-user install summary, preserves user-owned Agent Gateway configuration, atomically installs/upgrades the bundle, owns verify/uninstall controls, and enables the hardened user-systemd service. The complete path is validated in a clean Fedora desktop VM through the laptop mock LLM. A privacy-safe local status API is also implemented. Public packaging is blocked on an address-capable Agent Gateway listener or equivalent local-only transport, then publisher signing; application-profile UI, trust workflow, binary download/update, and graphical UI also remain pending.
