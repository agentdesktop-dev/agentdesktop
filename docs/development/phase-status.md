...
- Phase 2: partial. The connector retains experimental DPoP forwarding while the production enrollment path uses ordinary OAuth for user identity and short-lived mTLS certificates for device identity. Agent Desktop reads an explicit enrollment-service URL from organization bootstrap, generates and protects a P-256 private key, submits a signed CSR with bearer authentication, verifies the authority's public-key fingerprint, polls owner-scoped status, persists the returned chain, rejects a leaf certificate whose SPKI does not match its key, and fails managed startup closed unless reqwest/rustls can load and present the approved identity. OAuth pool rotation reapplies mTLS identity. The Go service validates issuer-pinned ES256 or RS256 bearer tokens and signed P-256 CSRs, persists pending enrollment in PostgreSQL, requires a separately scoped administrator token for bounded organization-scoped listing, approval, rejection, and device revocation, atomically claims issuance, creates client-auth certificates with authority-controlled SPIFFE identity, reconciles interrupted issuance with a stable request identity, atomically revokes devices and their certificates, and withholds provisional device identity until approval completes. Rust tests exercise CSR signature and fingerprint binding, bearer authentication, protected persistence, certificate-key matching, reqwest identity loading, and managed pool rotation. Go unit tests and a disposable PostgreSQL 17 integration test cover request idempotency, audit, owner isolation, administrator lifecycle actions, duplicate transition rejection, retry-stable issuance, reconciliation, transactional completion, provisional-state isolation, and revocation organization isolation. Production CA integration, renewal, expired-certificate recovery, fail-closed revocation consumption, and Agent Gateway mTLS identity remain incomplete.
...
- Production enrollment still requires protected-key CA integration, renewal, expired-certificate recovery, and fail-closed Agent Gateway consumption of revocation state.
