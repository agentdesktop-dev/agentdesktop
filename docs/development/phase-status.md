...
- Phase 2: partial. The existing connector implements experimental browser PKCE login, DPoP-bound sessions, refresh rotation, protected storage, restart restoration, and enrollment fixtures. The selected production design now uses ordinary OAuth for user identity and short-lived mTLS certificates for device identity. A Go enrollment service validates issuer-pinned ES256 or RS256 bearer tokens, derives `(iss, sub)`, validates signed P-256 CSRs, applies PostgreSQL migrations, and transactionally persists pending enrollments. Unit tests and a disposable PostgreSQL 17 integration test cover this slice. Approval, CA issuance, renewal, recovery, revocation enforcement, Agent Desktop mTLS, and Agent Gateway mTLS identity remain incomplete.
...
- Production enrollment still requires administrator authentication and approval, a protected CA adapter, certificate issuance and renewal, expired-certificate recovery, and revocation enforcement.
...
Implement administrator approval and CA-backed client-certificate issuance in the Go enrollment service. Then add Agent Desktop key/certificate lifecycle and Agent Gateway mTLS validation before re-running the managed Fedora journey with attempts to override verified tunnel identity from inner application headers.