...
- Phase 5: in progress. Linux native forwarding and standalone transparent capture are validated in a disposable Fedora Workstation VM. All Rust libraries, binaries, and test targets compile warning-free for `x86_64-pc-windows-msvc` from Linux through a pinned `cargo-xwin` workflow; this is a compile check, not Windows runtime validation. The Windows QEMU harness has a validated Packer definition, unattended Windows 11 Enterprise installation, UEFI state, OpenSSH control, test-signing, host service forwarding, and disposable overlays. A real guest base build and clean overlay boot are validated. This development VM intentionally bypasses the Windows TPM and Secure Boot requirements because neither is part of the connector or WFP test boundary. The next slice makes native forwarding, managed identity, credentials, service lifecycle, trust, and installation work on Windows and macOS before adding platform capture. macOS development requires Apple hardware: disposable Virtualization.framework VMs for ordinary journeys and a physical Apple Silicon machine for System Extension approval and Network Extension lifecycle validation. CI automation is deferred while these developer-operated environments are established.
...
Execute native forwarding in the validated Windows guest. Then implement Windows credential storage, service startup, trust, and packaging. In parallel, prepare the Apple-hardware development environment for native macOS validation.