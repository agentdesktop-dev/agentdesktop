...
- Phase 3, partial transport and capture setup: application routing recognizes mutually exclusive native, connector, and captured choices; captured mode still fails before application launch because no original-destination listener is installed. A pooled HTTP/2 CONNECT primitive validates explicit destination ports and preserves bidirectional bytes. Linux setup deterministically generates atomic cgroup v2/nftables rules, redirects TCP/443, rejects UDP/443, and has private-container behavior coverage. Local/managed HBONE authentication, real Agent Gateway interoperability, original-destination forwarding, and production host validation remain blocked as documented below.
...
- Transparent capture still requires authenticated HBONE, original-destination recovery, and production OS validation. The selected first Linux mechanism is cgroup v2 with nftables, targeting an externally managed systemd scope, TCP/443 redirection, and UDP/443 denial. Private rootless-Podman coverage validates isolated rule behavior but cannot establish production host process attribution or anti-bypass guarantees. eBPF is documented as a future strengthening path.
...
- Linux original-destination listener, HBONE forwarding integration, and disposable-host tests; do not expose a captured application path before redirection and fail-closed denial work end to end.