...
- Verified device enrollment requires a production enrollment authority, authenticated administrator approval/revocation, durable connector records, and an agreed Agent Gateway device-identity contract. The repository's mock authority and client make the proposed protocol executable but are not a production trust source. A connector-generated DPoP key remains connector-instance proof and is not labeled as organizational device identity until approval is enforced end to end.
