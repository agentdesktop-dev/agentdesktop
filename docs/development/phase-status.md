...
- Native managed forwarding propagates certificate-derived user/device identity from the authenticated outer CONNECT connection into immutable inner request metrics in the managed VM example with Agent Gateway v1.4.1. Managed transparent capture remains blocked on proving the same propagation; the standalone local capability must never be used for managed transport.
...
Package and production-sign the Windows native WFP path, then add process-scoped launch gating and UDP denial before claiming Windows capture. In parallel, prepare the Apple-hardware development environment for native macOS validation and publish and consume certificate revocation state in Agent Gateway before verifying fail-closed rejection prior to certificate expiry.
