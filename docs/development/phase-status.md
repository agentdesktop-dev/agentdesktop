...
- Phase 2, native managed path: complete for the manual walkthrough. Agent Desktop uses ordinary OAuth bearer tokens for user identity and short-lived mTLS certificates for device identity, protects P-256 enrollment and retry-stable renewal keys, and rotates OAuth and device credential generations independently. It renews within six hours of expiry and recovers the latest certificate for seven days after expiry using OAuth plus enrolled-key proof. The Go service terminates TLS 1.3, permits certificate-free initial enrollment, verifies presented client certificates, persists and reconciles enrollment, renewal, and recovery state in PostgreSQL, and atomically revokes devices and certificates. Agent Gateway validates and removes the connector bearer token, requires a device certificate, calls the control plane over exact-identity service mTLS without caching, and injects provider credentials. The control plane independently verifies the submitted leaf and authorizes only its OAuth owner, active device, and current valid unrevoked certificate. Full automation of the real Gateway walkthrough and managed transparent-capture identity propagation remain pending.
...
Add Agent Gateway mTLS validation, current-generation and revocation checks, and immutable certificate-derived device context, then test it against Agent Desktop's managed upstream pool.