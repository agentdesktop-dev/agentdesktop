...
- Phase 2: partial. The existing connector implements experimental browser PKCE login, DPoP-bound sessions, refresh rotation, protected storage, restart restoration, and enrollment fixtures. The selected production design uses ordinary OAuth for user identity and short-lived mTLS certificates for device identity. The Go service validates issuer-pinned ES256 or RS256 bearer tokens and signed P-256 CSRs, persists pending enrollment in PostgreSQL, requires a separately scoped administrator token for approval, atomically claims issuance, and creates client-auth certificates with authority-controlled SPIFFE identity through a narrow CA interface. Unit tests and a disposable PostgreSQL 17 integration test cover request idempotency, audit, duplicate approval rejection, completion, and CA-failure rollback. Production CA integration, certificate delivery, interrupted-issuance reconciliation, renewal, recovery, revocation enforcement, Agent Desktop mTLS, and Agent Gateway mTLS identity remain incomplete.
...
- Production enrollment still requires protected-key CA integration, certificate delivery, interrupted-issuance reconciliation, renewal, expired-certificate recovery, administrator rejection/listing, and revocation enforcement.
...
Implement authenticated certificate retrieval plus interrupted-issuance reconciliation, then replace the local CA key with a production protected-key adapter. After that, add Agent Desktop key/certificate lifecycle and Agent Gateway mTLS validation before re-running the managed Fedora journey.