...
- Phase 2: partial. The existing connector implements experimental browser PKCE login, DPoP-bound sessions, refresh rotation, protected storage, restart restoration, and enrollment fixtures. The selected production design uses ordinary OAuth for user identity and short-lived mTLS certificates for device identity. The Go service validates issuer-pinned ES256 or RS256 bearer tokens and signed P-256 CSRs, persists pending enrollment in PostgreSQL, requires a separately scoped administrator token for approval, atomically claims issuance, creates client-auth certificates with authority-controlled SPIFFE identity through a narrow CA interface, reconciles interrupted issuance with a stable request identity, and lets only the authenticated enrollment owner poll status and retrieve the public certificate chain. Unit tests and a disposable PostgreSQL 17 integration test cover request idempotency, audit, owner isolation, duplicate approval rejection, retry-stable issuance, reconciliation, and transactional completion. Production CA integration, renewal, expired-certificate recovery, revocation enforcement, Agent Desktop mTLS, and Agent Gateway mTLS identity remain incomplete.
...
- Production enrollment still requires protected-key CA integration, Agent Desktop certificate installation, renewal, expired-certificate recovery, administrator rejection/listing, and revocation enforcement.
...
Replace the local CA key with a production protected-key adapter that honors the issuance idempotency key. After that, add Agent Desktop key/certificate lifecycle and Agent Gateway mTLS validation before re-running the managed Fedora journey.