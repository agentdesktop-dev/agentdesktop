...
- Phase 2: partial. The connector retains experimental DPoP forwarding while the production enrollment path uses ordinary OAuth for user identity and short-lived mTLS certificates for device identity. Agent Desktop reads an explicit enrollment-service URL from organization bootstrap and managed runtime configuration, generates and protects P-256 enrollment and retry-stable renewal keys, verifies authority fingerprints and leaf-key binding, persists approved credentials, and fails managed startup closed unless reqwest/rustls can present the identity. It renews within six hours of expiry and recovers the latest certificate for seven days after expiry using OAuth plus an enrolled-key signature over a one-use CSR-bound challenge. Replacements are persisted and reloaded before publishing a new device generation, and the upstream pool rotates independently for OAuth or device credential changes. The Go service terminates TLS 1.3 directly, permits certificate-free initial enrollment, verifies presented client certificates against the enrollment CA, persists enrollment, renewal, and recovery state in PostgreSQL, reconciles interrupted issuance with stable request identities, and atomically revokes devices and certificates. Rust and Go tests cover proof binding, cross-language signature encoding, retry-stable drafts and claims, owner isolation, lifecycle actions, reconciliation, and disposable PostgreSQL 17 integration. Production CA integration, fail-closed revocation consumption, and Agent Gateway mTLS identity remain incomplete.
...
- Production enrollment still requires protected-key CA integration and fail-closed Agent Gateway consumption of revocation state.
