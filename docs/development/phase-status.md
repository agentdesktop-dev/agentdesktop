...
- Phase 2, managed identity path: complete for the manual walkthrough. OAuth authenticates enrollment and bounded expired-certificate recovery. The authority issues one short-lived mTLS certificate whose SPIFFE URI binds the verified organizational user and device; managed forwarding carries no OAuth bearer token. Agent Desktop protects P-256 enrollment and retry-stable renewal keys, renews within six hours of expiry, rotates the HBONE pool after validated replacement, and recovers for seven days after expiry using OAuth plus enrolled-key proof. The Go service persists and reconciles enrollment, renewal, recovery, and revocation state in PostgreSQL. Agent Gateway validates the client certificate on its outer CONNECT listener, exposes it to captured policy as `source.tunnel.identity`, and injects provider credentials. Published revocation consumption remains pending.
