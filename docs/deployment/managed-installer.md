...
The managed installer is suitable for SSH-based development that simulates per-user MDM installation. MDM installation only places files, an inactive user-systemd unit, and the manifest-owned `~/.local/bin/agentdesktop` command link. It does not open a browser, request device enrollment, start forwarding, modify Claude settings, or edit shell startup files. The user later runs the printed `agentdesktop connect-agents` command. That command signs in through the browser, requests and waits for authority approval, starts the connector, verifies local readiness, and separately asks before changing Claude settings.
