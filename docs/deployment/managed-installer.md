...
The managed installer is suitable for SSH-based development that simulates per-user MDM installation. MDM installation only places files and an inactive user-systemd unit. It does not open a browser, request device enrollment, start forwarding, or modify Claude settings. The user later runs the printed `agentgateway-edge-connector connect-agents` command. That command signs in through the browser, requests and waits for authority approval, starts the connector, verifies local readiness, and separately asks before changing Claude settings.
