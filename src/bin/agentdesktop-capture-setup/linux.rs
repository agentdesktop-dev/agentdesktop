...
fn capture_lock() -> Result<fs::File> {
    ...
    rustix::fs::flock(&lock, FlockOperation::LockExclusive).context("lock Linux capture state")?;
    Ok(lock)
}
...
impl CaptureState {
    ...
    fn specs(&self) -> Result<Vec<CaptureSpec>> {
        let specs = self
            .registrations
            .iter()
            .map(|registration| CaptureSpec::new(&registration.cgroup, registration.redirect_port))
...
fn apply_and_save_capture_state(nft: &Path, state_path: &Path, state: &CaptureState) -> Result<()> {
...
mod tests {
    ...
    use super::{install_system_integration, install_trust, remove_trust};
